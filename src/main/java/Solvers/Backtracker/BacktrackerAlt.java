 ...
 * If seed %gt;= 0 then the sequence is random (the random number generator depends on the value of seed.)<br>
 ...
 * <br>
 * As soon as a new literal is selected and set locally to be true, further locally true literals may be derived,
...
public class BacktrackerAlt extends Solver {
    ...
    public static void makeSolvers(Parameters parameters,ArrayList<Solver> backtrackers) {
        ...
        if (arrangement1) {
            if(positiveFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 1, -1,1));
            if(negativeFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 1, -1,-1));}
        if (arrangement2) {
            if(positiveFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 2, -1,1));
            if(negativeFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 2, -1,-1));}
        if (arrangement3) {
            if(positiveFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 3, -1,1));
            if(negativeFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 3, -1,-1));}
        if (arrangement4) {
            if(positiveFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 4, -1,1));
            if(negativeFirst) backtrackers.add(new BacktrackerAlt(++solverNumber, 4, -1,-1));}
        for(int seed : seeds) {
            if(seed >= 0) {
