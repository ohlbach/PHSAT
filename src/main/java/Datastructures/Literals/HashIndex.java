...
public class HashIndex {
    ...
    public HashMap<Integer, ArrayList<CLiteralOld>> hashIndex = new HashMap<>();
    ...
    public void remove(CLiteralOld cLiteral) {
        ArrayList<CLiteralOld> cliterals = hashIndex.get(cLiteral.literal);
     ...
     */
    public ArrayList<CLiteralOld> get(int literal) {
    ...
    public void removeClauses(int literal) {
        ArrayList<CLiteralOld> cliterals = hashIndex.get(literal);
        if(cliterals == null) return;
        hashIndex.remove(literal);
        for(CLiteralOld cLiteral : cliterals) removeClause(cLiteral.clause);}
     ...
     */
    public void forEach(int literal, Consumer<CLiteralOld> consumer) {
        ArrayList<CLiteralOld> cliterals = hashIndex.get(literal);
        if(cliterals == null) return;
        for(CLiteralOld cLiteral : cliterals) consumer.accept(cLiteral);}
    ...
    public void setTimestamp(int literal, int timestamp) {
        ArrayList<CLiteralOld> cliterals = hashIndex.get(literal);
        if(cliterals != null) {
            for(CLiteralOld cLiteral : cliterals) cLiteral.clause.timestamp = timestamp;}}
     ...
     */
    public CLiteralOld findFirst(int literal, int timestamp) {
        ArrayList<CLiteralOld> cliterals = hashIndex.get(literal);
        if(cliterals == null)  return null;
        for(CLiteralOld cLiteral : cliterals) if(cLiteral.clause.timestamp == timestamp) return cLiteral;
        return null;}
}
