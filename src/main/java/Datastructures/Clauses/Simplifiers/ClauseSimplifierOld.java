...
public class ClauseSimplifierOld {
     ...
     */
    public ClauseSimplifierOld(ProblemSupervisor problemSupervisor, Monitor monitor, String monitorId, Thread thread) {
    ...
    public ClauseOld simplify(ClauseOld clause) throws Unsatisfiable {
     ...
     */
    protected ClauseOld replaceEquivalences(ClauseOld oldClause) {
        if(equivalenceClasses.isEmpty()) return oldClause;

        if(trackReasoning) {
            intList1.clear();
            ClauseOld newClause = oldClause;
            ...
            InferenceStep step = new EquivalenceReplacementsOld(oldClause,newClause, intList1,equivalenceClasses);
            newClause.inferenceStep = step;
            if(monitoring) monitor.print(monitorId,step.toString(symboltable));
            return newClause;}
        else{
            for(CLiteralOld cLiteral : oldClause) { // destructive replacements
     ...
     */
    protected ClauseOld simplifyOr(ClauseOld oldClause) throws Unsatisfiable {
        intList1.clear();
        ClauseOld newClause = oldClause;
        for(int i = 0; i < newClause.size(); ++i) {
     ...
     */
    protected ClauseOld simplifyAtleast(ClauseOld clause) throws Unsatisfiable  {
     ...
     */
    protected ClauseOld simplifyAtleastDestructively(ClauseOld clause) throws Unsatisfiable {
     ...
     * @throws Unsatisfiable if a contradiction is found.
     */
    protected ClauseOld simplifyAtleastCloning(ClauseOld clause) throws Unsatisfiable {
     ...
     * @throws Unsatisfiable if a contradiction was found.
     */
    protected ClauseOld simplifyAtmost(ClauseOld clause) throws Unsatisfiable  {
        return trackReasoning ?
     ...
     * @throws Unsatisfiable if a contradiction is found.
     */
    protected ClauseOld simplifyAtmostDestructively(ClauseOld clause) throws Unsatisfiable {
        removeSuperfluousLiteralsDestructively(clause);
        int quAmount = clause.quAmount;
     ...
     * @throws Unsatisfiable if a contradiction is found.
     */
    protected ClauseOld simplifyAtmostCloning(ClauseOld clause) throws Unsatisfiable {
        clause = removeSuperfluousLiteralsCloning(clause);
        int quAmount = clause.quAmount;
        if(quAmount >= clause.size()) {return null;}  // atmost 3 p,q  is always true
        if(quAmount <= 0) {                           // atmost 0 p,q -> false(p,q)
     ...
     * @throws Unsatisfiable if a contradiction was found.
     */
    protected ClauseOld simplifyExactly(ClauseOld clause) throws Unsatisfiable  {
        return trackReasoning ?
     ...
     * @throws Unsatisfiable if a contradiction is found.
     */
    protected ClauseOld simplifyExactlyDestructively(ClauseOld clause) throws Unsatisfiable {
        removeSuperfluousLiteralsDestructively(clause);
        int quAmount = clause.quAmount;
     ...
     * @throws Unsatisfiable if a contradiction is found.
     */
    protected ClauseOld simplifyExactlyCloning(ClauseOld clause) throws Unsatisfiable {
        clause = removeSuperfluousLiteralsCloning(clause);
        int quAmount = clause.quAmount;
        if(quAmount > clause.size()) {throw new Unsatisfiable(clause);} // exactly 3 p,q is false
        if(quAmount == clause.size()) {     // exactly 3 p,q,r -> true(p,q,r)
     ...
     */
    private void removeSuperfluousLiteralsDestructively(ClauseOld clause) {
     ...
     */
    protected ClauseOld removeSuperfluousLiteralsCloning(ClauseOld newClause) throws Unsatisfiable {
        ClauseOld oldClause = newClause;
        intList1.clear();
