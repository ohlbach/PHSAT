...
public class CNFTransformerTest {
    ...
    public void atleastToCNF()  {
        ...
        ClauseOld c1 = new ClauseOld( new int[]{1,atL,3,1,2,3,4,5});
        ArrayList<ClauseOld> cnf = ct.toCNF(c1);
        assertEquals("[1: 1,2,3, 2: 1,2,4, 3: 1,2,5, 4: 1,3,4, 5: 1,3,5, 6: 1,4,5, 7: 2,3,4, 8: 2,3,5, 9: 2,4,5, 10: 3,4,5]",cnf.toString());
        ClauseOld c2 = new ClauseOld( new int[]{2,atL,2,1,2,3,4,5,6});
        ids[0] = 0;
        cnf = ct.toCNF(c2);
        ...
        */
        ClauseOld c3 = new ClauseOld( new int[]{3,atL,2, 1,2,3,2,1});
        ids[0] = 0;
        cnf = ct.toCNF(c3);
    ...
    public void atmostToCNF()  {
        ...
        int[] ids = new int[]{0};
        CNFTransformer ct = new CNFTransformer(true,(()->++ids[0]));
        ClauseOld c1 = new ClauseOld( new int[]{1,atM,2,1,2,3});
        ArrayList<ClauseOld> cnf = ct.toCNF(c1);
        assertEquals("[1: -1,-2,-3]",cnf.toString());

        c1 = new ClauseOld( new int[]{2,atM,2,1,2,3,4});
        cnf = ct.toCNF(c1);
        assertEquals("[2: -1,-2,-3, 3: -1,-2,-4, 4: -1,-3,-4, 5: -2,-3,-4]",cnf.toString());

        c1 = new ClauseOld( new int[]{3,atM,2,1,2,3,3});
        cnf = ct.toCNF(c1);
        assertEquals("[6: -1,-3, 7: -2,-3]",cnf.toString());

        c1 = new ClauseOld( new int[]{4,atM,3,1,2,3,4,5,6});
        cnf = ct.toCNF(c1);
